...
function Dashboard() {
  ...
  const loadHistory = async (id) => {
    try {
      ...
      const formattedMessages = historyLogs.reverse().flatMap(log => [
      ...
      setMessages([
        { role: 'ai', content: `Identity verified for dataset "${id}". Neural history synchronized.` },
        ...formattedMessages
      ]);
    } catch (err) {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        role: 'ai',
        content: `Dataset "${file.name}" is successfully ingested. 

I have generated some suggested operations for you below based on your data. You can select one, or provide a custom prompt. I will build and update the dashboard according to your instructions!`
      }]);
      setIsChatOpen(true);
    } catch (error) {
  ...
  const handleAction = async (msg) => {
    ...
    try {
      ...
      if (response.data.viz === 'dashboard') {
        ...
        const newVizzes = (response.data.data.visualizations || []).map(v => ({
          ...v,
          isChatGenerated: msg !== "dashboard summary"
        }));
        setVisualizations(newVizzes);
        setCurrentAnalysis(null);
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden flex flex-col bg-slate-950">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {datasets.length > 0 ? datasets.map((d, i) => (
                  <div key={i} className="p-4 bg-white/5 rounded-2xl border border-white/5 group hover:border-indigo-500/30 transition-all">
                    ...
                    <button
                      onClick={() => { setFileId(d.filename); setIsHistoryOpen(false); }}
      ...
      <main className="flex-1 overflow-y-auto p-8 max-w-[1600px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20 px-1">
          {visualizations.map((v, i) => (
            <motion.div
              ...
              className={`glass-panel min-h-[350px] md:h-[450px] p-6 flex flex-col ${v.isChatGenerated ? 'border-indigo-500/40 shadow-lg shadow-indigo-500/5' : ''
                }`}
            >
              ...
              <div className="flex-1 w-full h-full bg-black/20 rounded-2xl overflow-hidden border border-white/5 p-2">
        ...
        {/* <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-8 mb-20 px-1">
